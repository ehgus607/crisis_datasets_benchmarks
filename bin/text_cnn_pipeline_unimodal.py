...
if __name__ == '__main__':
    ...
    parser.add_option("--nb_epoch", action="store", dest="nb_epoch", default=3000, type="int")
    parser.add_option("--max_seq_length", action="store", dest="max_seq_length", default=25, type="int")
    parser.add_option("--patience", action="store", dest="patience", default=200, type="int")
    parser.add_option("--patience-lr", action="store", dest="patience_lr", default=20, type="int")
    ...
    #base_name = os.path.splitext(base_name)[0]
    timestr = datetime.now().strftime("%d-%m-%Y_%I-%M-%S") #time.strftime("%Y%m%d-%H%M%S")
    ...
    train_x, train_y, train_le, train_labels, word_index, tokenizer = data_process.read_train_data(train_file,
                                                                                                   ...
                                                                                                   MAX_SEQUENCE_LENGTH,
                                                                                                   delim)
    dev_x, dev_y, dev_le, dev_labels, _ = data_process.read_dev_data(dev_file, tokenizer, MAX_SEQUENCE_LENGTH, delim,
                                                                     train_le)
    test_x, test_y, test_le, test_labels, _ = data_process.read_dev_data(test_file, tokenizer, MAX_SEQUENCE_LENGTH,
                                                                         delim,
                                                                         train_le)
